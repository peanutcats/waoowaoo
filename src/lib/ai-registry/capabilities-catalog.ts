...
export function resolveGenerationOptionsForModel(input: {
  ...
  const defaults = input.capabilityDefaults?.[input.modelKey]
  const runtime = input.runtimeSelections

  // Configured values are fixed, not runtime-overridable defaults.
  const selection = mergeSelectionRecords(runtime, defaults)
