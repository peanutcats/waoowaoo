...
The customized `docker-publish.yml` workflow is triggered by explicit `custom-api-*` release tags or manual dispatch, and is restricted to `peanutcats/waoowaoo`. It builds both images on native `ubuntu-24.04-arm` runners for this installation's `linux/arm64` architecture. It publishes commit-specific tags, records actual digests in the job summary/log, and never deploys to a local machine. The fork's `main` branch does not need to be replaced to publish a feature-branch snapshot.
